...
export function AppProductCard({ app }: AppProductCardProps) {
  return (
    ...
    >
      ...
      <h3 className="text-foreground font-display text-xl font-bold tracking-normal">{app.name}</h3>
      <p className="text-muted-foreground mt-3 text-base leading-relaxed">{app.tagline}</p>
      <span className="text-foreground mt-6 inline-flex text-sm font-semibold opacity-0 transition-opacity group-hover:opacity-100 group-focus-visible:opacity-100">
        View app
      </span>
    </Link>
  );
